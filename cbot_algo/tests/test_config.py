"""Config validation tests (port of xauusd_bot's test_config.py).

The core property under test: bad config must raise at construction
time, never clamp silently. In the cBot port the schema raises
ConfigValidationError (a ValueError) instead of pydantic's
ValidationError — same fail-fast semantics, no Rust dependency.
"""
...
def _unvalidated_filter(max_spread):
    """Bypass FilterConfig's own gt=0 check to exercise BotConfig's own
    live-environment guard rail (the pydantic original used
    model_construct for the same purpose)."""
    f = FilterConfig.__new__(FilterConfig)
    f.max_spread_points = max_spread
    f.duplicate_order_debounce_seconds = 5
    f.allowed_sessions = None
    return f
